...
public class XPathParser {
  ...
  private Document createDocument(InputSource inputSource) {
    ...
    try {
       ...
       * 解释：此处“验证”是指 xml 规范中定义的验证解析器，实际上仅控制 dtd 验证；是否开启XML文档的DTD验证
       * 其他：XML 的 DTD 验证文件由 EntityResolver  加载获取
       */
      ...
      // 指定此工厂生成的解析器将CDATA节点转换为文本节点并将其附加到相邻的文本节点(如果有的话)。
      factory.setCoalescing(false);
      /**
       * 功能：指示是否将工厂配置为生成扩展实体引用节点的解析器
       * 解释：非法的 XML 字符必须被替换为实体引用（entity reference）
       * 举例： < 必须写成 &lt; 在 XML 中有 5 个预定义的实体引用
       */
